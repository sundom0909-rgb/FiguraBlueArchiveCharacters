                    ...
                    callbacks = {
                        onPreAnimation = function (self)
                            if not self.exSkill.exSkills[2].init then
                                ...
                                self.exSkill.exSkills[2].stairs:setPos(6, 0, 6)
                                self.exSkill.exSkills[2].stairs:setRot(0, 180, 0)
                                self.exSkill.exSkills[2].stairs:setBlock(self.parent.compatibilityUtils:checkBlock("minecraft:oak_stairs"))
                                self.exSkill.exSkills[2].stairs:setVisible(false)
                                if host:isHost() then
                        ...
                        onAnimationTick = function (self, tick)
                            ...
                            elseif tick == 100 then
                                ...
                                self.exSkill.exSkills[2].stairs:setVisible(true)
                    ...
                    callbacks = {
                        onPreAnimation = function (self)
                            ...
                            for i = 1, 100 do
                                ...
                                self.exSkill.exSkills[3].penLightSwingOffsets[i] = math.random()
                            end
                        ...
                        onAnimationTick = function (self, tick)
                            ...
                            elseif tick == 69 then
                                ...
                                events.RENDER:register(function (delta)
                                    for i = 1, 100 do
                                        models.models.ex_skill_3.Stage.PenLights["PenLight"..i]:setRot(0, 0, math.sin((self.exSkill.exSkills[3].penLightSwingOffsets[i] + delta * 0.1) * 2 * math.pi) * 40)
                                    end
                            ...
                            elseif tick == 148 then
                                for _ = 1, 4 do
                                    table.insert(self.exSkill.exSkills[3].particleAnchors, {vectors.rotateAroundAxis(math.random() * 360, 0, math.random() * 1.5 + 0.5, 1.5, 0, 1, 0), vectors.hsvToRGB(math.random() * 0.28 + 0.9, 0.5, 1), math.random()})
                                end
                            ...
                            if tick >= 69 then
                                for i = 1, 100 do
                                    self.exSkill.exSkills[3].penLightSwingOffsets[i] = self.exSkill.exSkills[3].penLightSwingOffsets[i] + 0.1
                                    self.exSkill.exSkills[3].penLightSwingOffsets[i] = self.exSkill.exSkills[3].penLightSwingOffsets[i] > 1 and self.exSkill.exSkills[3].penLightSwingOffsets[i] - 1 or self.exSkill.exSkills[3].penLightSwingOffsets[i]
                                end
                                if (tick - 69) % 8 == 0 then
                            ...
                            if tick >= 148 and tick < 166 then
                                ...
                                for i = 1, 4 do
                                    local anchorPos = playerPos:copy():add(vectors.rotateAroundAxis(bodyYaw, self.exSkill.exSkills[3].particleAnchors[i][1]:copy():add(0, math.sin(((tick - 148) / 18 + self.exSkill.exSkills[3].particleAnchors[i][3]) * 8 * math.pi) * 0.25, 0), 0, 1, 0))
                                    particles:newParticle(self.parent.compatibilityUtils:checkParticle("minecraft:firework"), anchorPos):setScale(1):setVelocity(anchorPos:copy():sub(playerPos):normalize():mul(0.05, 0, 0.05)):setColor(self.exSkill.exSkills[3].particleAnchors[i][2]):setGravity(0):setLifetime(213 - tick)
                                end
                            end
                        ...
                        onPostAnimation = function (self, forcedStop)
                            ...
                            if forcedStop then
                                ...
                                self.exSkill.exSkills[3].penLightSwingOffsets = {}
                                self.exSkill.exSkills[3].particleAnchors = {}
                                if host:isHost() then
                                    events.RENDER:remove("ex_skill_3_background_render")
